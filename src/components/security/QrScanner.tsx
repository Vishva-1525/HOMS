    ...
    <div className="relative min-h-[min(52dvh,28rem)] flex-1 overflow-hidden bg-black">
      ...
      {active && starting && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/75">
          ...
          <p className="text-sm font-medium text-white/90">Starting scanner camera…</p>
        </div>
      )}
      ...
      {active && cameraError && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/90 p-6 text-center">
          <div className="max-w-xs">
            <p className="text-sm font-semibold text-white">Scanner camera unavailable</p>
            <p className="mt-2 text-sm leading-relaxed text-white/75">{cameraError}</p>
            <p className="mt-3 text-xs text-white/55">
              Connect the USB QR reader / camera, allow permission, then reload. Manual entry
              still works below.
            </p>
          </div>
        </div>
      ...
      {showCamera && (
        <div className="pointer-events-none absolute left-0 right-0 top-3 px-3 text-center">
          <p className="text-xs font-medium tracking-wide text-white/90">
            Live scanner view · aim at the student pass QR
          </p>
          {cameraLabel && (
            <p className="mt-1 truncate text-[10px] text-white/60">{cameraLabel}</p>
          )}
        </div>
      )}
    </div>
